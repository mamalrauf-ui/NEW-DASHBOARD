...
client = get_client()
option_like, all_methods = find_option_method(client)
...
with st.sidebar:
    ...
    if not option_like:
        ...
        method_name = st.selectbox("انتخاب متد دستی", all_methods)
    else:
        method_name = st.selectbox("متد داده‌ی آپشن", option_like, index=0)
